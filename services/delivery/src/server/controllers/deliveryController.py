...
def addPayload():
    ...
    return deliveryService.addPayload(body["customerName"], body["customerMail"], int(body["finalPosition"]), body["x"],
                                      body["y"], body["satellite"])
...
def setStatus():
    body = request.get_json()
    # print(body["rocketName"])
    return deliveryService.setStatus(body["rocketName"], body["satelitte"])
